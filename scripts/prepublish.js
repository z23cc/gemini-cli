...
if (!fs.existsSync(packageJsonPath)) {
  ...
  if (
    ...
    (!packageJson.repository.url.includes('google-gemini/gemini-cli') &&
     !packageJson.repository.url.includes('z23cc/opengemini-cli'))
  ) {
    errors.push(
      `Error: The "repository" field in ${packageJsonPath} must be an object pointing to a valid git repository.`,
    );
  }
}
